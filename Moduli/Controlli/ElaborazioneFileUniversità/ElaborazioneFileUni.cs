            ...
            foreach (var filePath in excelFiles)
            {
                ...
                using (SqlDataReader reader = readData.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        if (codFiscale == "TRTDCH02H13L273O")
                        {
                            string test = "";
                        }
            ...
            void ProcessTipoCorso(DataRow row, StudenteElaborazione studente, string uniType)
            {
                ...
                switch (uniType)
                {
                    ...
                    case "ISIA":
                        ...
                        else if (cellToProcess.Contains("BIENNALE") || cellToProcess.Contains("MAGISTRALE"))
                        {
                            studente.tipoCorsoUni = "5";
                        }
                    ...
                    case "UNICUS":
                        if (cellToProcess != "")
                        {
                            studente.tipoCorsoUni = cellToProcess;
                        }
                        else
                        {
                            //ERRORE
            ...
            void ProcessAnnoImmatricolazione(DataRow row, StudenteElaborazione studente, string uniType)
            {
                ...
                try
                {
                    ...
                    switch (uniType)
                    {
                        ...
                        case "FOROITA":
                        case "CAMPUSBIO":
                            string annoSuccessivo = (int.Parse(cellToProcess) + 1).ToString();
                            annoImmatricolazione = cellToProcess + annoSuccessivo;
                            break;
                        case "SAPIENZA":
