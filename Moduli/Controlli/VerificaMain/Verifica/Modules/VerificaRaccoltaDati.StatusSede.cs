...
(
    SELECT
        ...
        CodFiscale AS CodFiscale,
        COALESCE(TipoBando,'') AS TipoBando,
        CAST(ISNULL(CodTipoEsitoBS,0) AS INT) AS CodTipoEsitoBS,
        CAST(ISNULL(ImportoAssegnato,0) AS DECIMAL(18,2)) AS ImportoAssegnato,
...
(
    ...
    JOIN D on D.NumDomanda = mm.Num_Domanda
),
...
(
    SELECT
        CodFiscale, ComuneDomicilio, TitoloOneroso, ContrattoEnte, TipoEnte, SerieContratto,
        DataRegistrazione, DataDecorrenza, DataScadenza, DurataContratto, Prorogato, DurataProroga,
        SerieProroga, DenomEnte, ImportoRataEnte
    FROM DOM_LRS
    WHERE rn = 1
),
IST_OPEN AS
...
(
    SELECT
        CodFiscale, NumDomanda, NumIstanza, CodTipoIstanza, ComuneDomicilio, TitoloOneroso, ContrattoEnte,
        TipoEnte, SerieContratto, DataRegistrazione, DataDecorrenza, DataScadenza, DurataContratto, Prorogato,
        DurataProroga, SerieProroga, DenomEnte, ImportoRataEnte
    FROM IST_OPEN
    WHERE rn = 1
),
IST_CLOSED_LAST AS
...
(
    SELECT
        CodFiscale, NumDomanda, NumIstanza, CodTipoIstanza, EsitoIstanza, UtentePresaCarico
...
";
            ...
            IReadOnlyDictionary<StudentKey, StudenteInfo>? infoByKey = null)
        {
            ...
            using var scope = MeasureCollectionStep("VerificaRaccoltaDati.StatusSedeReader", $"AA={aa}");
